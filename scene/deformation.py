...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, skips=[], args=None, distill_feature=False):
    ...
    def change_net(self, args):
        self.grid = HexPlaneField(args.bounds, args.kplanes_config, args.multires)
        mlp_out_dim = 0
        feature_out = [nn.Linear(mlp_out_dim + self.grid.feat_dim ,self.W)]
        self.feature_out = nn.Sequential(*feature_out)

    def change_net1(self, args):
        self.grid.feat_dim = args.kplanes_config['output_coordinate_dim'] * len(args.multires)
        feature_out = [nn.Linear(self.grid.feat_dim ,self.W)]
        self.feature_out = nn.Sequential(*feature_out)
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, time_emb):
        ...
        if self.distill_feature:
            return pts, scales, rotations, opacity, hidden
        else:
...
class deform_network(nn.Module):
    def __init__(self, args, distill_feature=False) :
        ...
        self.deformation_net = Deformation(
            W=net_width, D=defor_depth, input_ch=(4+3)+((4+3)*scale_rotation_pe)*2, 
            input_ch_time=timenet_output, args=args, distill_feature=distill_feature)
    ...
    def get_mask(self):
        for i1, item1 in enumerate(self.deformation_net.grid.grids):
            for i2, item2 in enumerate(item1):
                mask = item2 > 0.25
                self.deformation_net.grid.grids[i1][i2] = self.deformation_net.grid.grids[i1][i2] * mask                
        return
    ...
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, times_sel=None):
        ...
        if self.distill_feature:
            means3D, scales, rotations, opacity, hidden = self.deformation_net(point,
                                                    scales,
                                                    rotations,
                                                    opacity,
                                                    times_sel)
            return means3D, scales, rotations, opacity, hidden
        else:
