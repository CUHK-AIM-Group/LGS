...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False, load=True):
        """b
        ...
        device = self.gaussians._deformation.deformation_net.grid.aabb.device
        self.gaussians._deformation.deformation_net.grid.set_aabb(xyz_max,xyz_min)
        self.gaussians._deformation.to(device)

        if self.loaded_iter and load:
