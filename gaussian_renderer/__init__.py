...
def render(
    ...
    deformation_feature = None

    if stage == "coarse" :
        means3D_deform, scales_deform, rotations_deform, opacity_deform = means3D, scales, rotations, opacity
    elif distill_feature and stage == "fine":
        means3D_deform, scales_deform, rotations_deform, opacity_deform, deformation_feature = pc._deformation(means3D[deformation_point], scales[deformation_point], 
    ...
    return {
        ...
        "radii": radii,
        "deformation_feature": deformation_feature
    }
...
def count_deformation_render(
    ...
    """
    Render the scene.
    Count by considering the deformation
    Background tensor (bg_color) must be on GPU!
    """
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = (
    ...
    deformation_point = pc._deformation_table
    if not distill_feature:
        means3D_deform, scales_deform, rotations_deform, opacity_deform = pc._deformation(means3D[deformation_point], scales[deformation_point], 
                                                                         rotations[deformation_point], opacity[deformation_point],
                                                                         time[deformation_point])
    else:
        means3D_deform, scales_deform, rotations_deform, opacity_deform, _ = pc._deformation(means3D[deformation_point], scales[deformation_point], 
                                                                         rotations[deformation_point], opacity[deformation_point],
                                                                         time[deformation_point])
    with torch.no_grad():
        pc._deformation_accum[deformation_point] += torch.abs(means3D_deform - means3D[deformation_point])
    ...
    return {
        ...
        "important_score": important_score,
        "opacity": opacity,
        "delta_scale": torch.abs(torch.prod(scales.detach(), dim=1) - torch.prod(scales_final.detach(), dim=1))
        # "delta_scale": opacity,
    }
